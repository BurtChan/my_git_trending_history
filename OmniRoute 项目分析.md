...
**最新动态**：
- OmniRoute 持续快速迭代，已从 v3.0.0 的 67+ 提供商扩展至 268+ 提供商、500+ 模型，覆盖 Claude、GPT、Gemini、Kimi K3、GLM、DeepSeek 等主流模型
- 免费层大幅扩展：50+ 免费提供商（包括 Kiro 无限 Claude、Qoder AI 无限 DeepSeek R1、Pollinations 无需 API Key 的 GPT-5），每月免费额度达 16 亿 Tokens
- 新增 4 层智能回退机制（订阅 → API Key → 低价 → 永久免费），13 种路由策略和 Auto-Combo AI 智能路由
- 社区爆发式增长：500+ 贡献者参与开发，2,500+ 测试用例，版本迭代至 v3.8.49
- 生态集成持续扩展：新增 OpenCode 插件、14 种 CLI Agent 支持、25 个 MCP 工具、A2A 协议支持、Desktop/PWA 应用

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 22,804 | 25,057 | +2,253 |
| 总 Forks | 2,850 | 3,092 | +242 |

**核心变化概要**：
- 提供商从 67+ 扩展至 268+（+300%），模型从 200+ 增至 500+
- 免费提供商增至 50+，每月 16 亿免费 Tokens
- 新增 4 层智能回退和 13 种路由策略
- 500+ 贡献者，2,500+ 测试，版本迭代至 v3.8.49
- 新增 14 种 AI 编码工具支持、25 个 MCP 工具、Desktop/PWA 应用
...
**最新动态**：
- Stars 从 25,057 增长至 26,330（+1,273），Fork 从 3,092 增长至 3,293（+201），版本迭代至 v3.8.48
- 18 种路由策略 + 4 层自动故障转移（订阅 → API Key → 低价 → 免费），Combo 功能支持模型链式自动切换
- 10 引擎 Token 压缩管线（Session-Dedup → CCR → RTK → Headroom → Relevance → Caveman → LLMLingua-2 等），节省 15-95% token
- 新增 Quota-Share 配额公平分配功能，支持按权重/维度/策略在共享账户间分配配额
- 500+ 贡献者参与开发，已集成 268+ 提供商和 500+ 模型，成为 AI 编程工具生态中最活跃的网关项目

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 25,057 | 26,330 | +1,273 |
| 总 Forks | 3,092 | 3,293 | +201 |
