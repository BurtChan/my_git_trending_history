...
**最新动态**：
OmniRoute 自今年 2 月发布以来增长迅猛，目前已迭代至 **v3.8.49**（270+ 个 release）。核心升级包括：支持的 AI 提供商从早期版本扩展至 **290+**（其中 **90+ 提供免费额度**，40+ 永久免费），覆盖模型达 **500+** 个；月度免费 token 总量约 **15.3 亿**（来自 43 个提供商池、516 个模型的文档化免费额度）。

路由策略从基础 fallback 演进为 **19 种路由策略**（含 priority、weighted、round-robin、p2c、cost-optimized、context-relay、fusion、pipeline 等），并推出旗舰功能 **Combo**——跨模型自动链式路由，配额耗尽或提供商故障时静默切换。压缩引擎升级为 **12 级引擎栈**（Session-Dedup、CCR、Lite、RTK、Caveman、Aggressive、LLMLingua-2、OmniGlyph 等），RTK + Caveman 叠加后平均节省 **89.2%** token（78.4%–94.6%）。YouTube 上多个技术频道（AI Stack Engineer 等）发布教程视频，单条播放量超 2.6 万，显著推动社区增长。500+ 贡献者参与开发，项目已从个人工具发展为社区驱动的开源 AI 网关生态。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 27,204 | 29,127 | +1,923 |
| 总 Forks | 3,565 | 3,565 | - |

**核心变化概要**：
- 提供商扩展至 290+（90+ 免费），模型达 500+，月免费 token 约 15.3 亿
- 路由策略增至 19 种，推出 Combo 跨模型自动链式路由功能
- 12 级压缩引擎栈，RTK + Caveman 叠加节省 89.2% token
- 迭代至 v3.8.49（270+ release），500+ 贡献者参与
- YouTube 技术教程推动社区快速增长
## 总结
...
### 更新 3 — 2026 年 7 月 29 日（再次登上 Trending）
**更新原因**：项目再次登上 GitHub Trending 榜单

**最新动态**：
OmniRoute 经历了爆发式的一周增长，Stars 从约 22.8K 飙升至约 32.9K（**+10,100**），今日单日新增 **1,929** 颗 Star。YouTube 上多个技术频道（AI Stack Engineer、CoddyKit、knightli 等）发布教程视频，单条播放量超 2.6 万，显著推动了社区增长。v3.8.49 版本已迭代至 **12 级压缩引擎栈**，新增 OmniGlyph 实验性上下文编码引擎和 Responses Tool Output 无损 JSON 压缩引擎。RTK + Caveman 叠加后平均节省 **89.2% token**（78.4%–94.6%），支持 **290+ 供应商、500+ 模型**，兼容 **33+ 个 CLI/Agent 工具**，**104 个 MCP 工具**，**25,000+ 测试用例**。500+ 贡献者参与开发，项目已成为 AI 网关领域最活跃、最全面的开源项目。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 22,804 | 32,900 | +10,096 |
| 总 Forks | 2,800 | 4,300 | +1,500 |

**核心变化概要**：
- Stars 从 22.8K 增长至 32.9K（+10,100），本周爆发式增长
- 12 级压缩引擎栈上线，新增 OmniGlyph 上下文编码引擎
- YouTube 技术教程和博客评测显著推动社区增长
- 500+ 贡献者，104 MCP 工具，33+ CLI/Agent 工具兼容
- 新增 Cloudflare Workers AI 边缘推理和 AWS Polly 语音合成集成
