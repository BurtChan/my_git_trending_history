# Claude-Red 项目分析

## 项目名称
**Claude-Red** — 面向 Claude Skills 体系的进攻性安全技能库
- **GitHub**: [SnailSploit/Claude-Red](https://github.com/SnailSploit/Claude-Red)
- **许可证**: MIT

---

## 项目概述
Claude-Red 是一个精心策划的进攻性安全（offensive security）技能库，专门为 Claude 的 Skills 系统设计。每个技能都是一个结构化的 SKILL.md 文件，通过 frontmatter 元数据和专家级方法论文本，将 Claude「调教」为特定攻击领域的资深专家——从 SQL 注入到 shellcode 编写，从 EDR 绕过到漏洞利用开发。

项目由安全社区 SnailSploit 维护，2026 年 3 月创建后快速迭代，从最初 38 个技能扩展到 58 个、再到当前 100+ 技能规模（目标约 130 个技能、23+ 分类），是 Claude Skills 生态在安全攻防领域最系统的扩展库之一。它的出现代表了 AI 安全工作流的一个新范式：不再是「人用工具」，而是「AI Agent 按技能卡片的专家方法论自主执行侦察、利用、后渗透全流程」。

需要注意的是，该项目定位为专业红队/渗透测试人员的效率倍增器，技能内容按 MITCK ATT&CK 战术编号组织（如 TA0001 初始访问），强调合法授权测试场景。

---

## 核心功能

| 分类 | 代表技能 | 覆盖内容 |
|------|----------|----------|
| 初始访问 | `offensive-initial-access` | 钓鱼载荷、drive-by 投递、供应链投毒向量（TA0001） |
| 红队全链 | `offensive-advanced-redteam` | C2 基础设施、OPSEC、横向移动、持久化 |
| EDR 绕过 | `offensive-edr-evasion` | 用户态 unhooking、间接系统调用、PPID 欺骗 |
| Shellcode | `offensive-shellcode` | 编写、编码、注入技术、位置无关代码 |
| 无线攻防 | `offensive-wifi` 系列 | 802.11 侦察、WPA2-PSK 握手捕获/PMKID、WPA3-SAE Dragonblood、802.1X/EAP 凭据中继 |
| 移动安全 | `offensive-mobile` | 移动端攻击面方法论 |

---

## 技术栈
| 组件 | 技术 |
|------|------|
| 技能格式 | SKILL.md（frontmatter 元数据 + Markdown 方法论正文） |
| 目标平台 | Claude Code / Claude Skills 体系 |
| 组织标准 | MITCK ATT&CK 战术分类 |
| 语言 | Python（辅助脚本） |

---

## 项目亮点

### 专家方法论的「技能化」
每个 SKILL.md 不是简单提示词，而是一套完整的攻击方法论卡片：包含前置知识、工具链选择、分步流程、常见坑点。Claude 加载技能后即具备该领域的专家级推理路径，大幅降低红队工程师在多攻击面之间切换的认知成本。

### 覆盖面系统化
从 Web（SQLi）、二进制（shellcode）、终端对抗（EDR evasion）到无线（WiFi 全家族）和移动端，按 ATT&CK 战术编号组织，可按杀伤链阶段组合使用，形成「初始访问→横向移动→持久化」的完整技能流水线。

### 社区驱动的快速演进
从 38 → 58 → 100+ 技能的扩张速度，加上明确的贡献规范（单一攻击面技能优先于大而全概览），保证了技能库的深度与颗粒度持续提升。

---

## 应用场景

### 红队与渗透测试自动化
授权渗透测试中，让 Claude Code 加载对应技能后辅助生成载荷、分析侦察数据、规划攻击路径，测试人员聚焦决策。

### 蓝队反向训练
防御方研读进攻方法论技能卡，理解攻击者视角，用于制定检测规则与加固策略（项目在 r/blueteamsec 亦有讨论）。

### AI 安全研究
作为研究「LLM + 结构化攻击知识」能力的标本，观察技能化知识注入对模型行为的影响。

---

## Star 数据

| 指标 | 数值 |
|------|------|
| ⭐ 总 Stars | 5,622 |
| 🍴 总 Forks | 742 |
| 📈 今日新增 | 699 |
| 创建时间 | 2026-03-04 |
| 主要语言 | Python |
| 许可证 | MIT |

## 📋 更新记录

### 更新 1 — 2026年9月13日（连续第二天登上 Trending）

连续第二天登上 GitHub Trending，Star 从 3,326 增至 3,512（+186），Fork 从 532 增至 546，保持约 2.6% 的日增速，说明红队/渗透社区对 Claude Skills 攻防技能库的关注是持续性需求而非单日热度。

技能库按既定路线继续扩展（目标约 130 个技能、23+ 分类），MITCK ATT&CK 战术编号组织方式未变，项目在「AI Agent 按专家方法论自主执行攻防流程」这条路线上继续深耕。

**Star 数据**

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|------|------|------|
| 总 Stars | 3,326 | 3,512 | +186 |
| 总 Forks | 532 | 546 | +14 |
| 今日新增 | — | 186 | — |

**核心变化**

- Star 3,326 → 3,512（+186，日均 +186）
- Fork 532 → 546（+14）
- 连续在榜第二日，增长稳健，暂无异常波动信号

---

### 更新 2 — 2026年9月14日（连续第三天登上 Trending）

**更新原因**：连续第三天登上 GitHub Trending 榜单，单日新增 436 Stars（API 精确数据），增速较昨日翻倍以上。

**最新动态**：Claude-Red 连续第三天在榜，单日新增从昨日的 186 升至 436，热度不降反升，总 Stars 逼近 4,000。作为面向 Claude Skills 生态的进攻性安全（OffSec）技能库，其走红踩中了 Agent 安全方位的空白：随着 Skills 生态爆发，红队测试、提示注入防御等需求快速增长，安全类技能库成为稀缺供给。Forks 单日 +30，表明已有安全从业者开始基于其做二次封装。

**Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|------|------|------|
| 总 Stars | 3,512 | 3,948 | +436 |
| 总 Forks | 546 | 576 | +30 |

**核心变化概要**：

- 连续第三天在榜，单日 +436 较昨日（+186）翻倍以上
- 总 Stars 逼近 4,000，上线约一周即达此量级
- Agent 安全区块的技能库供给仍稀缺，卡位优势明显

---

### 更新 3 — 2026年9月15日（连续第四天登上 Trending）

Claude-Red 连续第四天登上 Trending：Star 从 3,948 增至 4,675（+727），Fork 从 576 增至 626（+50），单日增速为本轮在榜最高，红队攻防工具的需求热度仍在攀升。

作为面向 Claude 系模型的对抗性测试/红队平台，其在提示注入、越狱与安全评估场景的工具化能力持续被安全社区引用，四日累计 +1,349 星说明这类「模型安全基础设施」正从尝鲜转向日常工作流。

**Star 数据**

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|------|------|------|
| 总 Stars | 3,948 | 4,675 | +727 |
| 总 Forks | 576 | 626 | +50 |
| 今日新增 | — | 606 | — |

**核心变化**

- Star 3,948 → 4,675（+727），本轮在榜单日最高增速
- Fork 576 → 626（+50），二次开发与私有部署需求明显
- 连续第四天在榜，稳定占据安全赛道流量入口

---

### 更新 4 — 2026年9月17日（连续第六天登上 Trending）

**更新原因**：连续第六天登上 GitHub Trending 榜单，两日累计新增 947 Stars（API 精确数据）。

**最新动态**：Claude-Red 连续第六天在榜，总 Stars 从 4,675 增至 5,622（+947，跨两日增量），热度依然坚挺，突破 5.5K。作为面向 Claude Skills 生态的进攻性安全（OffSec）技能库，其走红持续踩中 Agent 安全方位的空白：Skills 生态爆发带来的红队测试、提示注入防御需求仍在增长，安全类技能库仍是稀缺供给。Forks 增至 742（+116），二次开发与私有封装需求明显。

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 4,675 | 5,622 | +947 |
| 总 Forks | 626 | 742 | +116 |
| 今日新增 | — | +699 | — |

**核心变化概要**：
- Star 4,675 → 5,622（+947，跨 9/15→9/17 两日），突破 5.5K
- Fork 626 → 742（+116），二次开发与私有部署需求明显
- 连续六天在榜，稳定占据安全赛道流量入口

---

## 总结
Claude-Red 把进攻性安全知识以 Claude Skills 的标准格式「技能化」，100+ 专家级方法论卡片覆盖从钓鱼到 EDR 绕过的完整杀伤链，是 AI Agent 深度参与红队工作流这一新范式的代表性项目。

---

*数据来源：GitHub 仓库 (SnailSploit/Claude-Red)，2026 年 9 月访问*
*首次分析：见文件头部 | 最近更新：2026 年 9 月 17 日*
