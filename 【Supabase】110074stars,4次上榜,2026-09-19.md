...
Supabase 是一个开源的 Postgres 开发平台，其愿景是"用企业级开源工具构建 Firebase 的功能"。它为开发者提供专用的 Postgres 数据库，搭配实时订阅、身份认证、对象存储、Edge Functions 等服务，构成一个完整的后端即服务（BaaS）平台。拥有超过 109,000 Stars 和 13,900+ Forks，是开源 BaaS 领域最受欢迎的项目，也是 Firebase 的最强开源替代。
...
### 更新 1 — 2026 年 9 月 16 日（时隔两个半月再次登上 Trending）

**更新原因**：时隔两个半月再次登上 GitHub Trending 榜单，总 Stars 从 105,250 增长至 109,413（+4,163），单日新增 118 Stars，稳居开源 BaaS 赛道星数榜首。

**最新动态**：Supabase 在 AI 应用后端场景的统治力持续加强——pgvector 向量检索、数据库 Advisors、Edge Functions 与主流 AI 框架（Vercel AI SDK、LangChain 等）的深度集成使其成为 AI 全栈开发默认选项之一。近两个半月净增超 4 千 Stars，Forks 同步突破 1.39 万，社区围绕迁移指南与自托管方案的讨论热度不减。

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 105,250 | 109,413 | +4,163 |
| 总 Forks | 12,957 | 13,973 | +1,016 |
| 今日新增 | 145 | 118 | -27 |

**核心变化**：
1. Stars 突破 10.9 万，两个半月净增 +4,163，日均约 55，长线增长稳健
2. Forks 从 12,957 增至 13,973（+1,016），生态采用度持续扩大
3. 定位从「Firebase 开源替代」进一步聚焦为「Postgres 开发平台」，AI 后端集成成为主要增长引擎


### 更新 2 — 2026 年 9 月 17 日（连续第二日在榜）

**更新原因**：连续第二日登上 Trending，总 Stars 从 109,413 增长至 109,655（+242），Forks 从 13,973 增至 14,026（+53）。

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 109,413 | 109,655 | +242 |
| 总 Forks | 13,973 | 14,026 | +53 |
| 今日新增 | 118 | 118 | — |

**核心变化**：
1. Stars 稳步净增 +242，Forks 突破 1.4 万，生态采用度继续扩大
2. AI 后端集成（pgvector / Edge Functions）仍是主要增长引擎


### 更新 3 — 2026 年 9 月 19 日（再次登上 Trending）

**更新原因**：再次登上 GitHub Trending 榜单，总 Stars 从 109,655 增长至 110,074（+419），正式突破 11 万关口，Forks 从 14,026 增至 14,251（+225）。

**最新动态**：AI 应用后端的采用热潮延续，pgvector 向量检索、Edge Functions 与主流 AI 框架的深度集成继续拉动 Supabase 成为 AI 全栈开发的默认后端选项之一。本轮回榜单日新增 128 Stars，增速较昨日（+242）的累积量进一步扩大，社区围绕自托管与迁移指南的讨论保持活跃。

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 109,655 | 110,074 | +419 |
| 总 Forks | 14,026 | 14,251 | +225 |
| 今日新增 | 118 | 128 | +10 |

**核心变化**：
1. Stars 净增 +419，突破 11 万里程碑（110,074），开源 BaaS 赛道星数榜首稳固
2. Forks 增至 14,251（+225），生态采用度持续扩大
3. 单日新增 128 高于昨日水平，回榜热度回升
...
*数据来源：GitHub 仓库 (supabase/supabase)，2026 年 9 月访问*
