...
### 更新 2 — 2026 年 9 月 15 日（连续第二日在榜）

**更新原因**：连续登上 GitHub Trending，今日新增 386 星，总星逼近 6,000，Agent 技能供应链安全赛道持续升温。

**最新动态**：
Agent Skills 项目在昨日新增 219 星后今日再增 386 星，增速不降反升，显示「AI 技能市场安全扫描」这一痛点正在被更广泛的开发者社区认知。项目以「13.4% 的市场技能有严重漏洞」的行业数据切入，静态分析 + 逐项扫描 + 一键安装到 15+ Agent 的全开源组合持续吸引关注。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 5,596 | 5,982 | +386 |
| 总 Forks | 499 | 508 | +9 |

**核心变化**：
- 总 Stars 达 5,982，单日新增 386 星，较昨日 +219 增速提升约 76%
- 总 Forks 增至 508，社区基于扫描器的二次开发开始出现
- 连续第二日在榜，Agent 技能供应链安全从边缘议题走向主流视野



**更新原因**：再次登上 GitHub Trending（昨日新建文件、无更新块，delta +219 > 100 首日阈值入选）

Agent Skills（Tech Leads Club）次日回榜：总 Stars 从 5,377 增至 5,596（+219），Forks 从 486 增至 499（+13）。作为「经过验证的 Agent 技能注册表」，其 CLI 双模式（React+Ink 交互式 TUI / Commander 非交互式）+ MCP 服务器双通道安装体验是核心卖点——技能从 CDN 注册表按需拉取（10 并发批量下载 + 本地缓存），SHA-256 内容哈希 + Zod 校验的 lockfile（v2）保证供应链可审计，「Agent 技能供应链安全」赛道随着技能生态爆发正成为刚需。

**Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 5,377 | 5,596 | +219 |
| 总 Forks | 486 | 499 | +13 |
| 开放 Issues | — | 30 | — |

**核心变化概要**：
- 总 Stars 5,377 → 5,596（+219），次日回榜
- Forks 486 → 499（+13）
- 开放 Issues 30 个，注册表生态进入实际使用反馈期
