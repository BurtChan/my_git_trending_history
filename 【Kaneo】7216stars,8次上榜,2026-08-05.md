...
### 更新 8 — 2026年8月5日（再次登上 Trending）
**更新原因**：Kaneo 持续登顶 Trending，开源项目管理工具热度不减，日增 565 Star

**最新动态**：Kaneo 作为 Jira/Linear 的开源替代方案，持续登上 GitHub Trending 榜单并曾达到 Trendshift TypeScript 排行榜第一。项目定位为「All you need, Nothing you don't」的极简项目管理工具，提供 Backlog、Board、List 和 Gantt 四种视图，支持工作区管理、成员邀请、标签、评论和 WebSocket 实时通知。技术栈基于 React + Hono，支持 Docker 自托管部署和 Railway 一键部署模板。项目新增 i18n 国际化支持，CLAUDE.md 和 .cursor/rules 配置文件表明已深度适配 Claude Code 和 Cursor 等 AI 编码工具。v2,313 commits 的活跃开发节奏显示项目正在快速迭代。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 6,811 | 7,216 | +405 |
| 总 Forks | 569 | 579 | +10 |

**核心变化概要**：
- 持续登上 Trending，Trendshift TypeScript 排行榜曾达第一
- 新增 i18n 国际化支持，适配全球用户
- 深度适配 Claude Code（CLAUDE.md）和 Cursor（.cursor/rules）
- Railway 一键部署模板上线，自托管门槛进一步降低
- 2,313 commits 活跃开发，React + Hono 技术栈成熟
