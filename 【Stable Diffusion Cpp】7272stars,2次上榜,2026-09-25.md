...
项目自 2023 年 8 月持续迭代至今（908 commits、691 个 release、113 位贡献者），基于 ggml 张量库构建，支持多种加速后端。近期动态密集：2026 年 9 月 23 日刚发布 master-908 版本，包含圆形 RoPE 重构与图像模型支持扩展（#2039）、Qwen Image 2.1 alpha 通道输入（#2021）、fp8 处理兼容上游 ggml（#2001）等改进。100% C++ 的单语言构成在 AI 项目中相当罕见。今日以 7,272 Stars、+36 增速登上 Trending。
...
连续第二天登上 Trending，Star 数从 7,160 增至 7,272（+112），Forks 809 → 813。昨日的 master-908 版本发布（圆形 RoPE 重构、Qwen Image 2.1 alpha 通道、fp8 兼容上游 ggml）仍在持续吸引关注，9 月 24 日仓库又有新推送，迭代节奏密集。

在各类「大模型本地化推理」项目中，stable-diffusion.cpp 是极少数专注图像/视频扩散模型且保持纯 C++ 单语言构成的存量项目，受益于 llama.cpp 生态的 GGUF 格式互通，用户迁移成本低。

**Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|---------|---------|------|
| 总 Stars | 7,160 | 7,272 | +112 |
| 总 Forks | 809 | 813 | +4 |
| Open Issues | — | 270 | — |

**核心变化**：

- 连续第 2 天在榜，日增 Star 112，增速较昨日（+33）明显放大，或与 master-908 新版本长尾传播有关
- Forks +4（809→813），Open Issues 270 个，社区互动稳定
- 仓库 9 月 24 日仍有推送，发布节奏保持在「每日构建」量级（691 个 release）
- 无新增 release，本阶段增长主要由 Trending 曝光驱动
...
*数据来源：GitHub 仓库 (leejet/stable-diffusion.cpp)，2026 年 9 月 25 日访问*
