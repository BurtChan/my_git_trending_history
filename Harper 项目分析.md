...
**最新动态**：
Harper 是由 Automattic（WordPress 母公司）维护的离线隐私优先语法检查器，使用 Rust 编写。2026 年 7 月发布了 v2.6.0 版本，性能进一步提升，特别是在初始加载和未缓存文档处理方面表现显著改善。项目采用 LSP（Language Server Protocol）架构，可无缝集成到各类编辑器中。Reddit 社区活跃讨论了 Harper 的最新开发进展，其在开发者工具领域的独特定位（隐私优先 + 高性能）受到广泛认可。代码库中 Rust 占比 78%，TypeScript 11.3%，Svelte 7.2%，展现出良好的多语言工程实践。累计发布 110 个版本，开发节奏稳定。
...
**核心变化概要**：
- v2.6.0 发布，初始加载和未缓存文档性能大幅提升
- 累计 110 个版本发布，开发节奏稳定
- LSP 架构支持多编辑器无缝集成
- Rust 78% + TypeScript 11.3% 的多语言技术栈
...
**最新动态**：
Harper 是由 Automattic 支持的隐私优先开源语法检查器，使用 Rust 编写。近期发布了 v0.41.0 大版本更新，主要改进了底层语法引擎的性能和准确性，同时包含 UI 相关优化。Automattic（WordPress 母公司）的收购和持续投入使 Harper 从一个独立项目成长为技术团队的标准语法工具。Founders UK 等媒体将其定位为「真正理解开发者工作方式的语法检查器」，强调其对代码注释和技术文档的深度支持。与通用语法工具不同，Harper 专为开发者和技术写作者设计，在隐私保护（完全离线运行）和计算效率方面具有独特优势。
...
**核心变化概要**：
- v0.41.0 发布，底层语法引擎大幅改进
- Automattic 持续投入，定位从独立项目升级为开发者标准工具
- 媒体和社区广泛报道，隐私优先 + 开源策略获得认可
