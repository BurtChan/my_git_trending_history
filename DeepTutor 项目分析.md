...
**最新动态**：
DeepTutor 以惊人的增长速度再次登上 Trending 榜单——总 Star 数从约 11,658 飙升至 27,540（增长 15,882 颗，增幅超过 136%），今日新增 531 颗。这一爆发式增长反映了项目近期一系列重大版本更新带来的广泛社区关注。

自上次分析以来，DeepTutor 发布了多个重要版本，其中最具里程碑意义的是 **v1.5.x 系列**：
- **v1.5.1**（2026.7.9）：新增从知识库中删除单个失败文档的功能，无需删除和重建整个知识库
- **v1.4.5**（2026.6.14）：重建引导学习系统，基于聊天 Agent 循环实现硬性掌握度门控，新增 `/learning` 仪表板
- **v1.4.4**（2026.6.13）：支持从 ClawHub 安装社区技能，具备安全门控机制，新增 DOCX/XLSX 原生预览
- **v1.4.3**（2026.6.12）：TutorBot 正式更名为 Partners，基于生产级 IM 管道（15 个频道）实现实时流式传输
- **v1.4.0-beta**（2026.5.21）：三层记忆工作台（L1/L2/L3），所有聊天能力基于单一 Agentic 引擎重建

项目已实现完全开源（v1.5），由香港大学数据智能实验室持续主导开发。DeepTutor 的 Agent-Native 架构设计正在成为 AI 教育领域的重要范式，其将多 Agent 协作、RAG 知识库、持久化记忆与自主导师系统深度融合的创新方案，获得了学术界和开源社区的广泛认可。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 11,658 | 27,540 | +15,882 |
| 今日新增 | — | 531 | — |
| 总 Forks | 1,614 | 3,661 | +2,047 |

**核心变化概要**：
- Star 数从 11,658 飙升至 27,540（+136%），爆发式增长
- v1.5.x 发布，项目完全开源，知识库管理更灵活
- Partners 系统支持 15 个频道的生产级 IM 管道
- 三层记忆架构（L1/L2/L3）+ 单一 Agentic 引擎重建
- ClawHub 社区技能市场上线，支持安全门控安装社区技能
