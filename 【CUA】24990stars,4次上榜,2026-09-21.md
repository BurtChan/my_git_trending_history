...
**更新原因**：时隔近 5 个月再次登上 GitHub Trending 日榜

**最新动态**：CUA 在 computer-use 2.0 方向快速演进：9 月中旬与 Typesafe AI 合作推出 **jev-use**（Jev + Cua Driver 的快速计算机使用方案，开发预览版覆盖 macOS/Windows/Linux）；v0.28.0+ 引入 **Skills over MCP**，代理可通过单条 stdio 连接加载 Cua Driver 技能；同时官方支持在 Cua Cloud Fleets 上运行 OSWorld 基准任务（官方 Ubuntu 镜像转 Fleet 桌面 + Cua Driver）。项目还启动了首位 GTM 负责人招聘，商业化加速。

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 14,100 | 23,841 | +9,741 |
| 总 Forks | 872 | 1,648 | +776 |

**核心变化**：
- 近 5 个月 Star 增长 +9,741（+69%），computer-use 赛道热度与项目迭代共振
- Cua Driver 定位升级：面向任意代理的后台计算机使用驱动 + 跨 OS Fleet
- jev-use（快速计算机使用）、Skills over MCP（v0.28.0+）、OSWorld on Fleets 三项新能力密集落地

### 更新 2 — 2026 年 9 月 20 日（连续第二日在榜）

**更新原因**：连续第二日登上 GitHub Trending 榜单，总 Stars 从 23,841 增长至 24,417（+576），今日单日 +859，热度持续攀升。

CUA（Computer-Use Agents）持续霸榜，作为 trycua 打造的开源计算机使用代理框架，正成为「AI 操控电脑」赛道的基础设施级项目。

项目提供 Apple Silicon 原生优化的 macOS/Linux 虚拟机编排能力，配合 LLM 驱动的代理可以完成截图-推理-点击的完整桌面自动化闭环，近期社区围绕多代理协同与远程浏览器控制的讨论明显增多。

今日 +859 的单日增速在 2 万星量级项目中相当突出，显示「桌面控制 AI 代理」正从实验性玩法走向生产级应用场景。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
| --- | --- | --- | --- |
| 总 Stars | 23,841 | 24,417 | +576 |
| 总 Forks | 1,648 | 1,681 | +33 |
| 今日新增 | +383 | +859 | +476 |

**核心变化**：

1. Stars 从 23,841 增至 24,417（+576），连续两日在榜且单日增速放大
2. Forks 从 1,648 增至 1,681（+33），围绕代理框架的二次开发持续活跃
3. MIT 许可 + Apple Silicon 原生支持，桌面自动化 RPA 场景适配度高

### 更新 3 — 2026 年 9 月 21 日（连续第三日在榜）

**更新原因**：连续第三日登上 GitHub Trending（第 4 次上榜），单日新增 573 Stars（API 精确数据）。

**最新动态**：CUA（开源 Computer-Use Agent 框架，让 AI 操控虚拟桌面/浏览器完成复杂任务）总 Stars 从 24,417 增至 24,990（+573），即将突破 25K。连续三日在榜且单日增量稳定在 500+，说明「开源桌面自动化 Agent」赛道热度从爆发转入持续吸流阶段。

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 24,417 | 24,990 | +573 |
| 总 Forks | 1,681 | 1,715 | +34 |
| 今日新增 | — | +573 | — |

**核心变化**：
1. 总 Stars 从 24,417 增至 24,990（+573），即将突破 25K
2. 连续三日在榜，增速稳定
3. Forks 持续增长，围绕代理框架的二次开发活跃
