...
**更新原因**：项目再次登上 GitHub Trending 榜单（232 stars today）

**最新动态**：
- Code Review Graph 持续巩固本地优先代码智能图谱的领先地位，Star 数从 7 月 22 日的 26.3K 增长至 28.8K（+2,577）
- 项目主打「只读真正需要的部分」：通过 Tree-sitter 构建代码结构图谱，为 AI 编码工具提供最小化上下文，中位数实现 82 倍上下文缩减
- MCP 1.0 标准兼容与 15+ 种 AI 编码工具集成持续完善，本地优先的代码智能方案获得越来越多开发团队的采用

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 26,255 | 28,832 | +2,577 |
| 总 Forks | 2,340 | 2,671 | +331 |

**核心变化概要**：
1. Star 数从 26,255 增长至 28,832（+2,577），稳步逼近 30K
2. Forks 从 2,340 增至 2,671（+331），企业级采用与集成案例持续增加
3. 本地优先 + MCP 的代码智能图谱方案在大型代码库 AI 审查场景中价值凸显
...
**最新动态**：
- Code Review Graph 连续第四日登上 Trending，Star 数从 28,832 增长至 28,986（+154），稳步逼近 30K 里程碑
- 本地优先的代码智能图谱方案持续获得认可：通过 Tree-sitter 构建代码依赖关系图，让 AI 编码工具「只读真正需要的部分」，中位数实现 82 倍上下文缩减
- MCP 1.0 标准兼容与 15+ 种 AI 编码工具集成进一步完善，企业级大型代码库审查场景中的落地案例持续增加

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 28,832 | 28,986 | +154 |
| 总 Forks | 2,671 | 2,681 | +10 |

**核心变化概要**：
1. Star 数从 28,832 增长至 28,986（+154），连续四日上榜，距离 30K 一步之遥
2. Fork 数从 2,671 增至 2,681（+10），企业级采用稳定增长
3. 本地优先 + MCP 的代码智能图谱定位在 AI 辅助审查场景中持续兑现价值
