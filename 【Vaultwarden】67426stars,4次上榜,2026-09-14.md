...
### 更新 3 — 2026 年 9 月 14 日（再次登上 Trending）
**更新原因**：项目再次登上 GitHub Trending 榜单，今日新增 +110 Stars

**最新动态**：
- 距上次记录（8 月 25 日）约三周，Star 从 66,071 增长至 67,426（+1,355），Fork 从 3,124 增至 3,206，自托管密码管理需求持续驱动稳定增长
- 项目保持高频安全更新节奏：5 月的 1.36.0 集中修复多个 SSO 安全通告（CSRF、用户枚举、SSRF），8 月发布 1.37.2 并配套 Web Vault 2026.7.0，官方提示需先升级服务端再升级 2026.7.0+ 客户端
- 1.37.x 系列落地 Send 功能与 SSO 绑定强化，并修复「未确认所有者可清空组织保管库」「跨组织组绑定越权读写」「安全戳轮换未失效刷新令牌」等多个安全通告，建议自托管用户尽快升级

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 66,071 | 67,426 | +1,355 |
| 总 Forks | 3,124 | 3,206 | +82 |
| 今日新增 | — | +110 | — |

**核心变化概要**：
- 安全版本密集发布（1.36.0 / 1.37.2），多个 GHSA 级别漏洞被修复
- Send 功能正式支持，Web Vault 同步至 2026.7.0
- Rust 轻量实现 + 全客户端兼容的组合仍是 homelab 社区事实标准
