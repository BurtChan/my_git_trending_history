...
### 更新 3 — 2026 年 8 月 10 日（再次登上 Trending）
**更新原因**：Prime Agent 连续第四日登上 GitHub Trending，Star 数从 8,444 增长至 10,674（+2,230），Forks 从 735 增至 1,106（+371）。项目开源仅一周即突破 1 万 Star 大关，增速持续领跑本周榜单。

**最新动态**：Prime Agent 热度持续爆发，开源一周即跨过 10K Star 里程碑，社区对其「自我改进 RLM Agent」架构的讨论热度不减。8 月 6 日开源后，项目迭代速度进一步加快，累计版本数已超过 41 个，v0.7.x 系列持续打磨工具调用与子 Agent 编排稳定性。Prime Intellect 官方博客详解的三大设计支柱——RLM 原生程序化工具调用、持久化多 Agent 编排、可自我修改的 Continual Harness——被多家技术媒体解读为「传统 context compaction 方案的替代路径」：工具、技能与子 Agent 全部以 Python 代码形式运行在单一持久 IPython 内核中，子 Agent 以函数调用形式异步返回结果，消息仅在父子兄弟实例间传递以避免跨会话干扰。ARC-AGI-3 基准 95.5%（178/183 关卡）的成绩持续被社区引用，多家评测机构开始将其作为「自我改进」类编码智能体的基准参照。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 8,444 | 10,674 | +2,230 |
| 总 Forks | 735 | 1,106 | +371 |

**核心变化概要**：
- Prime Agent 连续第四日登上 GitHub Trending，Star 数从 8,444 增长至 10,674（+2,230），Forks 从 735 增至 1,106（+371）。项目开源仅一周即突破 1 万 Star 大关，增速持续领跑本周榜单。
...
### 更新 4 — 2026 年 8 月 11 日（再次登上 Trending）
**更新原因**：Prime Agent 连续第五日登上 GitHub Trending，Star 数从 10,674 增长至 12,851（+2,177），Forks 从 1,106 增至 1,300（+194），开源仅一周多即逼近 1.3 万 Star，增速持续领跑榜单。

**最新动态**：Prime Agent 热度持续攀升，Star 数一周内从 5K 级跃升至 1.28 万，社区对其「自我改进 RLM Agent」架构的讨论不断深化。项目持续高频迭代，v0.7.x 系列版本陆续发布，累计版本数已超过 41 个。Prime Intellect 团队 8 月 5 日发布官方研究博客，系统阐述 RLM（Recursive Language Model）与 Continual Harness 两大设计支柱：RLM 将上下文视为可编程变量、子 Agent 委派视为 REPL 内的函数调用；Continual Harness 则允许 Agent 在运行时对自身提示词、技能、记忆与子 Agent 做 CRUD。社区热议的 ARC-AGI-3 基准 95.5%（178/183 关卡）成绩持续被引用，多家评测机构开始将其作为「自我改进」类编码智能体的基准参照，explainx.ai、kingy.ai 等媒体也陆续发布深度评测文章。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 10,674 | 12,851 | +2,177 |
| 总 Forks | 1,106 | 1,300 | +194 |

**核心变化概要**：
- Prime Agent 连续第五日登上 GitHub Trending，Star 数从 10,674 增长至 12,851（+2,177），Forks 从 1,106 增至 1,300（+194）
- v0.7.x 系列持续迭代，累计 41+ 个版本，开源一周即逼近 1.3 万 Star
- RLM + Continual Harness「自我改进」架构获 explainx.ai 等多家媒体深度评测
- ARC-AGI-3 95.5% 成绩持续被社区引用为基准参照
---

### 更新 5 — 2026 年 8 月 12 日（再次登上 Trending）

**更新原因**：Prime Agent 连续第六日登上 GitHub Trending，Star 数从 12,851 增长至 13,932（+1,081），Forks 从 1,300 增至 1,439（+139），开源仅一周多即逼近 1.4 万 Star。

**最新动态**：Prime Agent 热度不减，Star 数突破 1.39 万，增速继续领跑榜单。项目持续高频迭代，v0.7.x 系列版本继续推进，累计版本数持续增加。Prime Intellect 团队官方博客阐述的 RLM（Recursive Language Model）与 Continual Harness 两大设计支柱继续引发社区讨论——RLM 将上下文视为可编程变量、子 Agent 委派视为 REPL 内的函数调用；Continual Harness 允许 Agent 在运行时对自身提示词、技能、记忆与子 Agent 做 CRUD。ARC-AGI-3 基准 95.5%（178/183 关卡）的成绩持续被引用，多家评测机构将其作为「自我改进」类编码智能体的基准参照，explainx.ai 等媒体陆续发布深度评测。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 12,851 | 13,932 | +1,081 |
| 总 Forks | 1,300 | 1,439 | +139 |

**核心变化概要**：
- Star 数从 12,851 增长至 13,932（+1,081），逼近 1.4 万
- Forks 从 1,300 增至 1,439（+139）
- v0.7.x 系列持续迭代，「自我改进 RLM」架构讨论深化
- ARC-AGI-3 95.5% 成绩持续被引用为基准参照
...
*首次分析：2026 年 8 月 | 最近更新：2026 年 8 月 12 日*
