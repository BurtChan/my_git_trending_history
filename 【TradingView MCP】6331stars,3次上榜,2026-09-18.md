...
**最新动态**：TradingView MCP 自上次分析以来持续吸引社区关注，Star 数从 4,633 增长至 4,840。项目 issue 活跃度保持高位，近期社区提出了 BTC 宏观市场分类器（macro regime classifier）的功能请求，显示用户正在探索将 AI 代理与宏观经济分析相结合的更高级用例。此外还有多项 UI 同步问题和编辑器节点选择的 bug 报告，反映了社区在实际使用中发现并反馈真实场景问题。项目已被 SkillsLLM 收录为推荐 MCP 服务器技能，覆盖 Claude Code、Codex CLI 和 ChatGPT 等主流 AI 编码助手，进一步扩大了用户触达面。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 4,633 | 4,840 | +207 |
| 总 Forks | 2,161 | 2,174 | +13 |

**核心变化概要**：
- 社区提出 BTC 宏观市场分类器等高级功能请求
- 项目被 SkillsLLM 收录为推荐 MCP 服务器技能
- 持续修复编辑器节点选择和布局同步等实际使用问题
- Fork 数突破 2,170，社区参与度持续走高

---

### 更新 2 — 2026年9月18日（时隔约 2 个月再次登上 Trending）

**更新原因**：时隔约 2 个月再次登上 GitHub Trending 榜单。

**最新动态**：TradingView MCP 总 Star 从 4,840 增至 6,331（+1,491，约 +31%），Fork 从 2,174 增至 2,709。项目生态出现衍生分支：社区 fork「tradingview-mcp-jackson」在原版基础上新增 morning_brief（自选股晨报）、session_save/get（会话持久化）等规则化工作流工具，并修复 TradingView Desktop v2.14+ 启动问题，显示项目已成为「AI Agent × 交易分析」领域的事实基础。作者持续在 X 上推广 MCP+CLI 双形态用法。

**最新 Star 数据**：

| 指标 | 上次记录 | 最新数据 | 变化 |
|------|----------|----------|------|
| 总 Stars | 4,840 | 6,331 | +1,491 |
| 总 Forks | 2,174 | 2,709 | +535 |

**核心变化概要**：
- Star 增长 31%，Fork 突破 2,700，参与度极高
- 社区 fork（jackson 版）扩展晨报/会话持久化工作流
- MCP+CLI 双形态生态位稳固，金融 AI Agent 标杆案例
...
*数据来源：GitHub 仓库 (tradesdontlie/tradingview-mcp)，2026 年 9 月访问*
*首次分析：见文件头部 | 最近更新：2026年9月18日*